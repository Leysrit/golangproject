...
func (h *campaignHandler) UploadImage(c *gin.Context) {
	...
	if err != nil {
		errors := helper.FormatValidationError(err)
		errorsMessage := gin.H{"error": errors}
		response := helper.APIResponse("Failed to upload campaign image", http.StatusBadRequest, "error", errorsMessage)
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	response := helper.APIResponse("succesfully to upload campaign image", http.StatusOK, "success", data)
	c.JSON(http.StatusOK, response)
	return
}
